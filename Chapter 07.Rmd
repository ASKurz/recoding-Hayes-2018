...
Here is how to get the ungrouped $\mu$ and *SD* values for `justify` and `skeptic`, as presented in Table 7.3.
...
Here's the $R^2$ difference score for `model3` minus `model2`.
...
### Variable scaling and the interpretation of $b$~1~ and $b$~2~.
...
Here are the $R^2$ distributions for `model3` and `model4`. They're the same within simulaiton error.
