...
```{r models_1_and_2, cache = T, message = F, warning = F}
model1 <- 
  brm(data = glbwarm, 
      family = gaussian,
...
```{r correlations1_and_2_and_3, cache = T, message = F, warning = F}
correlations1 <- 
  brm(data = glbwarm, 
      family = gaussian,
...
For more details, you might also see the [28.11. Standardizing Predictors and Outputs subsection of the Stan User's Guide, Version 2.20](https://mc-stan.org/docs/2_20/stan-users-guide/standardizing-predictors-and-outputs.html)--[Stan](http://mc-stan.org), of course, being the computational engine underneath our **brms** hood.

### The effect of mean-centering on $b_1$, $b_2$, and their ~~standard errors~~ posterior $SD$s.

If you only care about posterior means, you can reproduce the results at the bottom of page 310 like this.
...
Here's what those summaries look like in a coefficient plot.
...
```{r}
posterior_summary(model6)[c("b_negemotzmissing_Intercept", "sigma_negemotzmissing"),] %>% 
  round(digits = 3)
```
...
```{r models8_and_9, cache = T, message = F, warning = F}
model8 <- 
  update(model7, 
         newdata = glbwarm,
...
```{r}
r2_without_age_interaction <- bayes_R2(model8, summary = F) %>% data.frame()
r2_without_sex_interaction <- bayes_R2(model9, summary = F) %>% data.frame()
r2_with_both_interactions  <- bayes_R2(model7, summary = F) %>% data.frame()
...
```{r}
nd <-
  tibble(negemot = seq(from = .5, to = 6.5, length.out = 30)) %>% 
  expand(negemot,
         sex = 0:1) %>% 
...
In the terms of `model7` where `sex` = $W$ and `age` = $Z$, we can restate that as
...
post %>% 
  # algebra
  transmute(men   = b_negemot + `b_negemot:sex` * 1 + `b_negemot:age` * 50,
            women = b_negemot + `b_negemot:sex` * 0 + `b_negemot:age` * 50) %>% 
  # more algebra
  mutate(`men - women` = men - women) %>% 
  gather() %>% 
  # this just orders the output
  mutate(key = factor(key, levels = c("men", "women", "men - women"))) %>% 
  group_by(key) %>% 
  mean_qi(value) %>% 
  mutate_if(is.double, round, digits = 3)
```

Switching our discussion to $b_5$ (i.e., $b_{\text{negemot} \times \text{age}}$), Hayes showed its value for two groups 10 years apart. Here it is for `model7`.
...
post %>% 
  transmute(women_50 = b_negemot + `b_negemot:sex` * 0 + `b_negemot:age` * 50,
            women_30 = b_negemot + `b_negemot:sex` * 0 + `b_negemot:age` * 30) %>% 
  mutate(`women_50 - women_30` = women_50 - women_30) %>% 
  gather() %>% 
  mutate(key = factor(key, levels = c("women_50", "women_30", "women_50 - women_30"))) %>% 
  group_by(key) %>% 
  mean_qi(value) %>% 
  mutate_if(is.double, round, digits = 3)
```
...
post %>% 
  transmute(men_50 = b_negemot + `b_negemot:sex` * 1 + `b_negemot:age` * 50,
            men_30 = b_negemot + `b_negemot:sex` * 1 + `b_negemot:age` * 30) %>% 
  mutate(`men_50 - men_30` = men_50 - men_30) %>% 
  gather() %>% 
  mutate(key = factor(key, levels = c("men_50", "men_30", "men_50 - men_30"))) %>% 
  group_by(key) %>% 
  mean_qi(value) %>% 
  mutate_if(is.double, round, digits = 3)
```
...
```{r model10, cache = T, message = F, warning = F}
model10 <- 
  update(model7, 
         newdata = glbwarm,
...
In the terms of `model10` where `sex` = $W$ and `age` = $Z$, we can restate that as
...
post %>% 
  # algebra
  transmute(men   = b_negemot + `b_negemot:sex` * 1 + `b_negemot:age` * 30 + `b_negemot:sex:age` * 1 * 30,
...
```{r}
post %>% 
  transmute(men   = b_negemot + `b_negemot:sex` * 1 + `b_negemot:age` * 50 + `b_negemot:sex:age` * 1 * 50,
...
post %>% 
  transmute(`30-year-old men`   = b_negemot + `b_negemot:sex` * w1 + `b_negemot:age` * z1 + `b_negemot:sex:age` * w1 * z1, 
...
```{r}
post %>% 
  transmute(contrast = `b_negemot:sex` * (w1 - w2) + `b_negemot:age` * (z1 - z2) + `b_negemot:sex:age` * (w1 * z1 - w2 * z2)) %>% 
  gather() %>%
  group_by(key) %>%
  summarize(mean = mean(value),
            sd   = sd(value),
