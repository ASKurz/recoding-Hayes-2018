...
Happily, the ochRe package has a handy convenience function, `viz_palette()`, that makes it easy to get a glimpse of the colors available in a given palette. We'll be using "olsen_qual" and "olsen_seq".
...
sapply(15:90, function(i){
  post$b_negemot + post$`b_negemot:age`*i
}) %>% 
  as_tibble() %>% 
  gather() %>% 
  mutate(age = rep(15:90, each = 4000),
         iter = rep(1:4000, times = 76)) %>% 
