...
We can fit our first moderation model like so.
...
If you want to follow along with Hayes on pate 276 and isolate the 95% credible intervals for the `skeptic` parameter, you can use the `posterior_interval()` function.
...
```{r loo_models_1_and_4, message = F, warning = F}
...
```{r}
post %>% 
  dplyr::select(Y_bar_12:Y_bar_24) %>% 
  gather() %>% 
  group_by(key) %>% 
  median_qi(value) %>% 
  mutate_if(is.double, round, digits = 3)
...
Hayes pointed out that the interaction effect, $b_3$, is the same across models his OLS Models 1 and 2. This is largely true for our Bayesian HMC `fit4` adn `fit5` models:
...
post %>% 
  dplyr::select(b_policy_kerry00:b_policy_kerry11) %>% 
  gather() %>%
