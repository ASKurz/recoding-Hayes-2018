...
```{r model2.1}
# save(model2.1, file = "fits/model02.01.rda")
# rm(model2.1)

load("fits/model02.01.rda")
```
...
To learn more about the multivariate syntax in **brms**, click [here](https://cran.r-project.org/web/packages/brms/vignettes/brms_multivariate.html) or execute `vignette("brms_multivariate")`.
...
```{r model2.3}
# save(model2.3, file = "fits/model02.03.rda")
# rm(model2.3)

load("fits/model02.03.rda")
```
...
```{r model2.4}
# save(model2.4, file = "fits/model02.04.rda")
# rm(model2.4)

load("fits/model02.04.rda")
```
...
```{r model2.5}
# save(model2.5, file = "fits/model02.05.rda")
# rm(model2.5)

load("fits/model02.05.rda")
```
...
```{r model2.6}
# save(model2.6, file = "fits/model02.06.rda")
# rm(model2.6)

load("fits/model02.06.rda")
```
...
```{r model2.7}
# save(model2.7, file = "fits/model02.07.rda")
# rm(model2.7)

load("fits/model02.07.rda")
```
...
```{r model2.8}
# save(model2.8, file = "fits/model02.08.rda")
# rm(model2.8)

load("fits/model02.08.rda")
```
...
Following Hayes on pages 50 and 51, here is the posterior mean, what you might call the Bayesian point estimate, for someone with:
...
```{r model2.9}
# save(model2.9, file = "fits/model02.09.rda")
# rm(model2.9)

load("fits/model02.09.rda")
```
...
```{r model2.10}
# save(model2.10, file = "fits/model02.10.rda")
# rm(model2.10)

load("fits/model02.10.rda")
```
...
```{r model2.11}
# save(model2.11, file = "fits/model02.11.rda")
# rm(model2.11)

load("fits/model2.11.rda")
```
...
```{r model2.12}
# save(model2.12, file = "fits/model02.12.rda")
# rm(model2.12)

load("fits/model02.12.rda")
```
...
```{r model2.13}
# save(model2.13, file = "fits/model02.13.rda")
# rm(model2.13)

load("fits/model02.13.rda")
```
...
[^1]: Hayes did not cover count regression with the Poisson likelihood in this textbook. If you'd like to learn more, one place to start is with [McElreath's lecture on binomial and Poisson models](https://www.youtube.com/watch?v=hRJtKCIDTwc).

[^2]: We will not be covering structural equation modeling (SEM) in this project. If you’re interested, you might check out [this video series](https://www.youtube.com/playlist?list=PLw93TUuxrFAZkJVc5dhgTZpOT7qmTjlT7) by Erin M. Buchanan on SEM within **R**, particularly with the handy [**lavaan**](http://lavaan.ugent.be/) package.
